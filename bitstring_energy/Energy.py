...
class IsingHamiltonian:
    ...
    def energy(self, bs: BitString) -> float:
        ...
        for i in range(len(bitString)):
            energy += self.mu[i] * bitString[i]
            for j in range(i+1, len(bitString)):
                energy += self.J[i,j] * bitString[i] * bitString[j]
        return energy
    def compute_average_values(self, time_steps) -> tuple:
        """
    ...
    def calculate_all_possible_energy(self, G: nx.Graph, 
        ...
        for b in range(0,  2**num_nodes+1):
            ...
            x_y_dict[b] = self.energy(current_bs, G)
